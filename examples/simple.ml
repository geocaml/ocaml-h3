...
let () =
  ...
  let austin =
    H3.lat_lng_to_cell { lat = austin_lat_rad; lng = austin_lon_rad } 12
  in
  printf "Austin: %Lx\n" austin;
  let testo =
    H3.lat_lng_to_cell { lat = d2r 40.689167; lng = d2r (-74.044444) } 5
  in
  printf "Testo: %Lx\n" testo;
  let austin_geo_out = H3.cell_to_lat_lng austin in
  printf "Austin Out Lat: %f Lon: %f\n" austin_geo_out.lat austin_geo_out.lng;
  printf "Austin Out Lat: %f Lon: %f\n"
    (H3.rads_to_degs austin_geo_out.lat)
    (H3.rads_to_degs austin_geo_out.lng);
